...
const supabase = window.KapilaAuth?.createClient?.() || (window.supabase?.createClient ? window.supabase.createClient(supabaseUrl, supabaseAnonKey) : null)
...
async function getAdminAccess() {
  if (!window.KapilaAuth?.getAdminAccess) return { role: null }
  const access = await window.KapilaAuth.getAdminAccess()
  return { role: access.role, user: access.user }
}
...
async function loadSubjects() {
  ...
  state.subjects = data || []
  if (!state.selectedSubjectId) state.selectedSubjectId = state.subjects[0]?.id || ''
}
...
async function loadChapters() {
  ...
  const { data, error } = await supabase
    ...
    .eq('subject_id', state.selectedSubjectId)
    .order('chapter_code')
  if (error) throw error
  state.chapters = data || []
  if (!state.selectedChapterCode) state.selectedChapterCode = state.chapters[0]?.chapter_code || ''
}
...
async function loadAll() {
  ...
  if (access.user) {
    if (roleTitle) roleTitle.textContent = 'Daily Learning'
    if (userEmail) userEmail.textContent = 'KAPiLA Management'
  }
...
function render() {
  ...
  dailyContent.innerHTML = `
    ...
    <div class="panel table-card">
      <p style="margin:0 0 12px 0;color:#475569">Selected Questions (${state.selectedQuestionIds.length})</p>
      <table>
        <thead>
          <tr>
            <th>Status</th>
        ...
        <tbody>
          ${selectedQuestions.length ? selectedQuestions.map((q) => `
            <tr>
          ...
          `).join('') : '<tr><td colspan="6">No selected questions.</td></tr>'}
        </tbody>
      </table>
    </div>

    <div class="panel table-card">
      <p style="margin:0 0 12px 0;color:#475569">All Questions</p>
      <table>
        <thead>
          <tr>
            <th>Status</th>
        ...
        <tbody>
          ${state.questions.length ? state.questions.map((q) => {
            const selected = selectedSet.has(q.id)
            return `
              <tr>
                <td><span class="kap-chip">${selected ? 'Selected' : 'Unselected'}</span></td>
                <td>${escapeHtml(q.question || '-')}</td>
                <td>${escapeHtml(q.quiz_id || '-')}</td>
                <td>${escapeHtml(q.chapter_code || '-')}</td>
                <td>${escapeHtml(q.difficulty_level || '-')}</td>
                <td>
                  <button class="kap-btn ${selected ? 'kap-btn-outline' : 'kap-btn-primary'}" data-toggle-id="${escapeHtml(q.id)}">
  ...
  dailyMeta.querySelectorAll('[data-action]').forEach((button) => {
    button.addEventListener('click', async () => {
      ...
      if (action === 'toggle-live') {
        ...
        const { data, error } = await supabase
          .from('kls_quizzes')
          .select('quiz_id,is_active,quiz_name,subject_id,chapter_code,difficulty_level,total_questions,time_limit')
          .eq('quiz_id', todayQuizId)
          .maybeSingle()
      ...
      if (action === 'publish') {
        ...
        const { error: deleteQuestionsError } = await supabase.from('kls_questions').delete().eq('quiz_id', todayQuizId)
        if (deleteQuestionsError) {
          state.publishing = false
          return setMessage(deleteQuestionsError.message)
        }
        const { error: deleteQuizError } = await supabase.from('kls_quizzes').delete().eq('quiz_id', todayQuizId)
        if (deleteQuizError) {
          state.publishing = false
          return setMessage(deleteQuizError.message)
        }
        const { error: deleteSettingsError } = await supabase.from('kls_daily_quiz_settings').delete().eq('quiz_id', todayQuizId)
        if (deleteSettingsError) {
          state.publishing = false
          return setMessage(deleteSettingsError.message)
        }

        const { error: insertQuizError } = await supabase.from('kls_quizzes').insert(payload)
        if (insertQuizError) {
          state.publishing = false
          return setMessage(insertQuizError.message)
        }

        const { error: insertSettingsError } = await supabase.from('kls_daily_quiz_settings').insert({
          quiz_id: todayQuizId,
        ...
        if (insertSettingsError) {
          state.publishing = false
          return setMessage(insertSettingsError.message)
        }

        const rowsToCopy = selectedQuestions.map((q) => ({
          quiz_id: todayQuizId,
          question: q.question,
          option_a: q.option_a,
          option_b: q.option_b,
        ...
        if (insertQuestionsError) {
          state.publishing = false
          return setMessage(insertQuestionsError.message)
        }

        await supabase.from('kls_daily_quiz_questions').delete().eq('quiz_id', todayQuizId)
        await supabase.from('kls_daily_quiz_questions').insert(
          state.selectedQuestionIds.map((id, index) => ({
            quiz_id: todayQuizId,
            question_id: id,
...
async function init() {
  ...
  const user = await window.KapilaAuth?.getSessionUser?.()
  if (user) {
    await loadAll()
  } else {
    showVisible(false)
