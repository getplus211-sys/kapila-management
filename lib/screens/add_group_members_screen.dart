...
class _AddGroupMembersScreenState extends State<AddGroupMembersScreen> {
  ...
  Future<void> _loadData() async {
    try {
      ...
      _existingMemberIds = members.map((m) => m['user_id'] as String).toList();

      // Load contacts
      final cached = _storage.getCachedContacts();
      if (cached != null) {
        final contacts = cached
            .where((c) => c['isRegistered'] == true && !_existingMemberIds.contains(c['userId']))
            .map((c) => ContactItem(
                  userId: c['userId'],
  ...
  Future<void> _addMembers() async {
    ...
    try {
      ...
      if (mounted) {
        ScaffoldMessenger.of(context).showSnackBar(
          SnackBar(content: Text('${_selectedUserIds.length} member(s) added')),
        );
    ...
    return Scaffold(
      ...
      appBar: AppBar(
        ...
        title: Column(
          ...
          children: [
            const Text('Add Members', style: TextStyle(fontWeight: FontWeight.w600)),
