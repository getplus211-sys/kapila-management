...
class _ChatWindowScreenState extends State<ChatWindowScreen>
  ...
  void _loadLocalMessages() {
    ...
    _messageIds = _messageMap.keys.toList()
      ..sort((a, b) =>
          _messageMap[a]!.createdAt.compareTo(_messageMap[b]!.createdAt));
    _findPinnedMessage();
    if (mounted) setState(() {});
  }
  ...
  Future<void> _fetchMessages() async {
    ...
    for (final msg in messages) {
      final local = LocalStorageService().getMessage(msg.messageId);
      // If user deleted it for themselves, never show it again
      if (local?.isDeletedForMe == true) continue;
      final effective = local ?? msg;
      _messageMap[effective.messageId] = effective;
    }
    _messageIds = _messageMap.keys.toList()
      ..sort((a, b) =>
          _messageMap[a]!.createdAt.compareTo(_messageMap[b]!.createdAt));
  ...
  void _subscribeToMessages() {
    _messageSubscription =
        _chatService.subscribeToMessages(widget.chatId).listen((newMessage) {
      final exists = _messageMap.containsKey(newMessage.messageId);
      final storage = LocalStorageService();
      final localBefore = storage.getMessage(newMessage.messageId);
      // Respect "delete for me" even if message is updated remotely
      if (localBefore?.isDeletedForMe == true) {
        return;
      }

      storage.saveMessage(newMessage);
      final effective = storage.getMessage(newMessage.messageId) ?? newMessage;

      if (!exists) {
        _messageMap[effective.messageId] = effective;
        _messageIds.add(effective.messageId);
        _messageIds.sort((a, b) =>
            _messageMap[a]!.createdAt.compareTo(_messageMap[b]!.createdAt));
        if (mounted) {
          setState(() {});
  ...
  void _subscribeToPresence() {
    _presenceSubscription = _chatService
        .subscribeToUserPresence(widget.otherUserId)
        .listen((updatedUser) {
      if (mounted)
        setState(() {
  ...
  void _subscribeToMessageStatus() {
    _statusSubscription = _chatService
        .subscribeToMessageStatus(widget.chatId)
        .listen((statusUpdate) {
  ...
  void _subscribeToTyping() {
    _typingSubscription = _chatService
        .subscribeToTyping(widget.chatId, widget.otherUserId)
        .listen((isTyping) {
      ...
      builder: (_) => SafeArea(
        ...
        _buildOptionTile(
            ...
            onTap: () async {
              ...
              setState(() {
                _messageMap.remove(message.messageId);
        ...
        if (isMe)
          _buildOptionTile(
              ...
              onTap: () {
                Navigator.pop(context);
                _showDeleteConfirmation(message, t);
              }),
        const SizedBox(height: 20),
      ]))),
    );
      ...
      builder: (_) => AlertDialog(
        ...
        content: TextField(
          ...
          decoration: InputDecoration(
            ...
            enabledBorder:
                OutlineInputBorder(borderSide: BorderSide(color: t.border)),
            focusedBorder:
                OutlineInputBorder(borderSide: BorderSide(color: t.brand)),
          ),
          maxLines: 3,
          autofocus: true,
        ),
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      ...
      floatingActionButton:
          _showScrollToBottom ? _buildScrollToBottomButton(t) : null,
    );
  }
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Text('${_otherUser?.fullName ?? 'User'} is typing...',
            style: TextStyle(
                fontSize: 12, color: t.text2, fontStyle: FontStyle.italic)),
      ]),
    );
  }
  ...
  PreferredSizeWidget _buildAppBar(ThemeProvider t) {
    ...
    if (_isSearching) {
      return AppBar(
        ...
        leading: IconButton(
            icon: Icon(Icons.arrow_back, color: t.text1),
        ...
        title: TextField(
          ...
          decoration: InputDecoration(
            ...
            suffixText: _searchResultCount > 0
                ? '$_currentSearchIndex of $_searchResultCount'
                : null,
    ...
    return AppBar(
          ...
          Expanded(
            ...
            children: [
              Text(
                ...
                style: TextStyle(
                    fontSize: 16, fontWeight: FontWeight.w600, color: t.text1),
              ...
              Text(
                ...
                style: TextStyle(
                    fontSize: 12,
                    color: (_isOtherUserTyping || isOnline)
                        ? Colors.greenAccent
                        : t.text2),
              ),
            ],
      ...
      actions: [
        IconButton(
            icon: Icon(Icons.search, color: t.text1), onPressed: _searchInChat),
        PopupMenuButton<String>(
          ...
          onSelected: (value) {
            if (value == 'view_profile') _openUserProfile();
          },
          itemBuilder: (_) => [
            PopupMenuItem(
                value: 'view_profile',
                child: Text('View Profile', style: TextStyle(color: t.text1))),
            PopupMenuItem(
                value: 'mute',
                child: Text('Mute', style: TextStyle(color: t.text1))),
            PopupMenuItem(
                value: 'clear_chat',
                child: Text('Clear Chat', style: TextStyle(color: t.text1))),
          ],
        ),
      ],
    ...
    return Container(
      ...
      child: Row(children: [
        ...
        Expanded(
          ...
          Text('Pinned Message',
              style: TextStyle(
                  fontSize: 11, fontWeight: FontWeight.bold, color: t.accent)),
    ...
    if (filteredIds.isEmpty) {
      return Center(
        ...
        Icon(Icons.chat_bubble_outline,
            size: 64, color: t.text2.withOpacity(0.3)),
    ...
    return Container(
      ...
      child: ListView.builder(
        ...
        itemBuilder: (_, index) {
          ...
          final message = _messageMap[messageId]!;
          final previousMsg =
              index > 0 ? _messageMap[filteredIds[index - 1]] : null;
          final isMe = message.senderId == _chatService.currentUserId;
          final showDateDiv = DateUtil.shouldShowDateDivider(
              previousMsg?.createdAt, message.createdAt);
          final showAvatar = previousMsg == null ||
              previousMsg.senderId != message.senderId ||
              showDateDiv;
          final replyToMsg = message.replyToMessageId != null
              ? _messageMap[message.replyToMessageId]
              : null;
