...
const nonEduStat = document.getElementById('nonEduStat')
...
function showSection(sectionName) {
  const activeSection = sectionName || 'dashboard'
...
async function loadBadges() {
  try {
    ...
    if (verificationStat) verificationStat.textContent = String(verificationPending)
    if (reportsStat) reportsStat.textContent = String(reportsPending)
    if (nonEduStat) nonEduStat.textContent = String(nonEducationalReportsPending)
  } catch (error) {
...
async function loadDashboardMeta(user, role) {
  if (sessionLabel) sessionLabel.textContent = role ? 'Admin Session' : 'Guest Mode'
  if (sessionSubLabel) sessionSubLabel.textContent = user?.email || (role ? 'Signed in' : 'Not signed in')
  if (sessionDetail) sessionDetail.textContent = user?.email || 'No active session.'
  if (roleDetail) roleDetail.textContent = role || '-'
  if (userEmail) userEmail.textContent = user?.email || 'Authenticated user'
  if (roleTitle) roleTitle.textContent = role ? `${role.toUpperCase()} Access Granted` : 'Login to continue'
  updateModuleVisibility(role)
  if (authScreen) authScreen.classList.toggle('hidden', Boolean(role))
  if (loginCard) loginCard.classList.toggle('hidden', Boolean(role))
  if (dashboardCard) dashboardCard.classList.toggle('hidden', !role)
  if (statusCard) statusCard.classList.toggle('hidden', !role)
}
...
function initModuleLinks() {
  moduleLinks.forEach((link) => {
    link.addEventListener('click', (event) => {
      ...
      setMessage(`${link.dataset.module} page opening...`, false)
    })
  })
}
...
async function init() {
  ...
  navLinks.forEach((link) => {
    link.addEventListener('click', () => {
      showSection((link.getAttribute('href') || '#dashboard').replace('#', ''))
    })
  })
